...
# figure 5 A and B
# ------------------------------------------------------------------------------
Fig5A <- top_figure_percent(top_number = 20)
Fig5A_legend <- get_legend(Fig5A)
Fig5A <- Fig5A + theme(legend.position = "none")
Fig5B <- top_figure_percent(top_number = 50)
...
Pred_Modelling$EL_cat_spec <- ifelse(Pred_Modelling$RankEL_minus > rankel_cutoff_90_speceficity, 1,0)
Pred_Modelling$EL_cat_cross <- ifelse(Pred_Modelling$RankEL_minus > rankel_cutoff_cross, 1,0)
...
All_samples_clinical <- Pred_Modelling_total_all_peptide %>%
  filter(RankEL_minus > rankel_cutoff_cross) %>% 
  group_by(Patient) %>%
...
ggdraw() +
  draw_plot(Fig5A,        .0, .8, .44, 0.2)+
  draw_plot(Fig5B,        .44, .8, .44, 0.2)+
  draw_plot(Fig5A_legend,        .84, .8, .2, 0.2)+
  draw_plot(p_rankel,        .0, .68, .31, 0.12)+
  draw_plot(Fig5C_cutoff_figure,        .335, .68, .31, 0.12)+
  draw_plot(Fig5C_cutoff_figure_TME,    .67, .68, .31, 0.12) +
  #draw_plot(Fig5C_cutoff_figure_legend, .46, 0.82, 0.1, 0.12) +
  draw_plot(Fig5C_legend, .46, 0.61, 0.1, 0.12) +
  ...
  draw_plot_label(c(paste0("MCC:",MCC_EL_cross),
                    paste0("MCC:",MCC_rf_cross),
                    paste0("MCC:",MCC_rf_tme_cross)),
                  x = c(0.11,0.44,.78), 
                  y = c(0.66,0.66,0.66), size = 16) + 
  
  draw_plot(CM_figure_EL_cross, .0, .5, .3, 0.14)+
 # draw_plot(CM_figure_EL_spec, .16, .5, .16, 0.14)+
  draw_plot(CM_figure_rf_cross, .335, .5, .3, 0.14) +
#  draw_plot(CM_figure_rf_spec, .5, .5, .16, 0.14) +
  draw_plot(CM_figure_TME_cross, .67, .5, .3, 0.14) +
#  draw_plot(CM_figure_TME_spec, .84, .5, .16, 0.14) +
  draw_plot_label(c("A","B","C","D","E"), x = c(0,0.44,.0,.0,.0), y = c(1,1,0.83,0.66,0.5), size = 22)  
...
test <- Pred_Modelling %>% 
  select(response_lab, prediction_rf,prediction_rf_tme,RankEL_minus) %>% 
  gather(., key = "model", value = "score" , -response_lab)

test %>%
  ggplot(., aes(x = model, y =score )) +
  geom_point(aes(color = response_lab)) +
  theme_bw()


Pred_modelling_test <- Pred_Modelling %>% 
  group_by(Patient,response) %>% 
  tally() %>% 
  filter(response == 1) %>% 
  mutate(eighty_procent = round(n*0.8,0)) %>% 
  select(Patient,eighty_procent) %>% 
  left_join(., Pred_Modelling)

Pred_modelling_test %>% filter(Patient=="MM-02") %>% select(eighty_procent)

ehight_persent_cutoff <- function(p = "MM-26", pred = "prediction_rf") {
 cutoff <-  Pred_modelling_test  %>%
  filter(Patient==p) %>% 
  filter(response == 1) %>% 
  top_n(eighty_procent[1],pred) %>%
    ungroup() %>% 
  select(pred) %>% 
  min() 
 return(cutoff)
...
pred_per_p_and_pred <- function(pred_name = "prediction_rf") {
  Pred_modelling_test_prediction_rf <- data.frame()
  Pred_modelling_test_number <- data.frame()
for (patient in unique(Pred_modelling_test$Patient)) {
  print(patient)
  cutoff <- ehight_persent_cutoff(p = patient, pred = pred_name)
  cut_off_vec <- c(patient, pred_name, cutoff)
  ehight_persent_cutoff_tab  <- rbind(ehight_persent_cutoff_tab,cut_off_vec)
  
  pred_per_p <- Pred_modelling_test %>% 
    filter(Patient == patient) 
  pred_per_p <- pred_per_p[pred_per_p[pred_name] >= cutoff,]
  pred_per_p$prediction_method = pred_name
 print(nrow(pred_per_p))
 Pred_modelling_test_number <- rbind(Pred_modelling_test_number,c(pred_name,patient,nrow(pred_per_p)))
# Pred_modelling_test_prediction_rf <- rbind(Pred_modelling_test_prediction_rf,pred_per_p)
}
  colnames(Pred_modelling_test_number) <- c("Prediction_method","Patient","number")
  return(Pred_modelling_test_number)
}

Pred_modelling_test_prediction_rf <- pred_per_p_and_pred(pred_name = "prediction_rf")
Pred_modelling_test_prediction_rf_tme <- pred_per_p_and_pred(pred_name = "prediction_rf_tme")
Pred_modelling_test_rankel <- pred_per_p_and_pred(pred_name = "RankEL_minus")
