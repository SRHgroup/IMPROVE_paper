...
Fig3C_2 <- Pred_Modelling %>% 
  ...
  geom_boxplot(aes(fill = response_lab),alpha = 0.5)+
  scale_y_continuous(limits = c(0,0.215))+
  theme_bw() +
  scale_fill_manual(breaks = c("no","yes"), values = response_col) +
...
unique(pr_all_dat$model)

SupFig3B  <- 
  pr_all_dat %>%  filter(model!="improve_tme") %>% 
...
Fig_3F_RankEL <- Pred_Modelling %>% filter(response==0) %>% 
  ...
  theme_bw()+
  scale_y_log10(limits = c(0.001, 50), breaks = c(0.001,0.005,0.1,1,5,15),
...
pdf(file = 'results/PaperPlots/Fig3/Figure3_ALL.pdf', width = 12, height = 12)
ggdraw() +
  ...
  draw_plot(Fig3B, .0, .0, 1, 0.43) +
  # draw_plot(Fig_3F_prediction_rf, .775, .17, .225, 0.25) +
  #  draw_plot(Fig_3F_RankEL,        .55, .17, .225, 0.25) +
  draw_plot_label(c("A","B","C","D"), x = c(0,0,.4,.0), y = c(1,0.81,0.81,0.45), size = 22)  
...
ggdraw() +
  ...
  draw_plot(SupFig3B, .65, .8, 0.3, 0.2) +
 # draw_plot(SupFig3C, .65, .74, 0.3, 0.152) +
  ...
  draw_plot_label(c("A","B","C","D","E","F"), x = c(0,0.64,.64,.0,0,0), y = c(0.99,0.99,0.82,0.617,0.427,0.237), size = 22)  
...
Fig4D <- plot_response(y_val ='MCPmean', y_label = "MCPmean",log = TRUE, limit = c(0.1,7000), map_level = T)
...
#  draw_plot(Fig4B_legend, .3, .7, 0.2, 0.24) +
  ...
  draw_plot_label(c("A","B","C","D","E","F"), x = c(0,0.19,0.39,0,0.59,0.38), y = c(1.005,1.005,1.005,0.74,1.005,0.74), size = 20)  
...
SupFig4C <- all_peptides %>%
  ...
  geom_quasirandom(aes(color = response_lab),size = 3) +
  geom_boxplot(aes(fill = response_lab), alpha = 0.5)+
  scale_y_log10( limits = c(4,24000)) +
  scale_color_manual(breaks = c("no","yes"), values = response_col) +
  facet_grid(.~HLA_type) +
  geom_signif(comparisons = list(c("no","yes")),
              na.rm = T,
...
SupFig4G_1 <- Pred_Modelling_immunogenic  %>% 
  ...
  theme(legend.position = "right", axis.title.y = element_text(size = 14),
        strip.text.x = element_text(size = 16),
        legend.text = element_text(size = 14),
        legend.title = element_text(size = 16)) +
...
ggdraw() +
  ...
  draw_plot(SupFig4A, .0 ,0.3, 0.4, 0.44) +
  
  
  draw_plot(SupFig4C , .4, .6, 0.5,0.32)  +
  draw_plot(SupFig4D , .4, 0.63, 0.2, 0.3) +
  draw_plot(SupFig4E, .7, .3, .3, 0.28) +
  
  draw_plot(SupFig4G_0, .0, .06, .337, 0.24) +
  draw_plot(SupFig4G_1, .44, .06, .337, 0.24) +
  draw_plot(SupFig4G_1_legend , .8, -.02, 0.1, 0.1)  +
