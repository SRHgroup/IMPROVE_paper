...
all_pep <- all_peptides %>% select(Mut_peptide,HLA_allele,response)
write.table(all_pep, file = "neoepitope_tabels/In_house_neoepitope_for_CV.tsv", sep = "\t", row.names = F, col.names = T, quote = F)
